...
def join(request):
    ...
    if request.method == 'POST':
        ...
        join_session = {
            ...
            'token':        generate_token(form.cleaned_data['username'])
        }
...
def verify_resend(request):
    ...
    if request.method == 'POST':
        ...
        send_activation_email(request, prof.user.email,
                              prof.user.username, prof.activ_token)
...
def edit_request_ajax(request, res):
    ...
    widget = forms.Textarea(
        attrs={'rows': 2, 'placeholder': 'I want to see ...', 'class': 'form-control'})
...
def request_status_ajax(request, res, status):
    ...
    # Only owner can change the request status
    req = validate_user_request(request, res)
    if not isinstance(req, Request):
        return req
...
def get_comments(request, req=None, userprof=None, num_pages=10, recent_first=False):
    """Util that returns comments in json format both for request and user
    profile."""
...
def update_password(request):
    ...
    if request.method == 'POST':
        ...
        form.is_bound = True
        pass_valid = ValidPassword(
            'The password is not valid', 'valid_password', user)
...
def get_location_requests(request, loc):
    """Returns requests for location which can be either city (City, State) or
    country."""
    # Not currently used
    reqs = WRequest.get_recent_requests()
...
def get_user_photos_json(request, username):
    ...
    for phreq in paged_phreqs:
        ...
        if is_you(request, userprof2.user):
            photo['remove_url'] = reverse(
                'remove_photo', args=(phreq.photo.id,))
        photos.append(photo)
...
def get_user_requests_json(request, username):
    ...
    reqs = Request.objects.filter(
        user=userprof2.user).order_by('-submitted_date')
    ...
    for item in req_items:
        ...
        if is_you(request, userprof2.user):
            data_item['remove_url'] = reverse(
                'remove_request', args=(req.resource,))
...
def submit_select(request, res):
    ...
    if request.method == 'POST':    # Ajax request
        ...
        try:
            ...
            # DB handling
            im = WImage.get_or_create_image_by_request(
                user, req, 'preview', marked_delete=True)
            im.name = filename
            ...
            logging.error(
                'Error uploading request (%s) photo preview: %s' % (res, e))
...
def submit_crop(request, res):
    ...
    result = handle_request_crop_file(
        req, user, im, (0, 0, im.width, im.height))
...
def handle_request_crop_file(req, user, image, (x, y, w, h)):
    ...
    imc = WImage.get_or_create_image_by_request(
        user, req, 'crop', marked_delete=True)
...
def profile_photo_select(request):
    ...
    if request.method == 'POST':    # Ajax request
        ...
        try:
            ...
            # DB handling
            im = WImage.get_or_create_profile_image(
                user, 'preview', marked_delete=True)
            im.name = filename
            im.height = img.height
            im.width = img.width
            im.is_avail = upload_file_avail
...
def profile_photo_crop(request):
    ...
    if im.width <= 140 and im.height <= 140:
        # No need to crop - upload directly
        result = handle_profile_crop_file(
            user, im, (0, 0, im.width, im.height))
        if isinstance(result, HttpResponseBadRequest):
            return result
