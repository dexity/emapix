...
class UserProfile(models.Model):
    ...
    b_month = models.CharField(
        max_length=12, choices=MONTH_CHOICES)   # Full month
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            if self.type == 'request':
                change_number(UserProfile, 'num_photos',
                              {'user': self.user}, 1)
...
class Image(models.Model):
    ...
    size_type = models.CharField(
        max_length=16, choices=IMAGE_SIZES, null=True, blank=True)
...
class Request(models.Model):
    ...
    # the photo request identification
    resource = models.CharField(max_length=16)
    status = models.CharField(
        max_length=1, choices=REQ_STATUS_CHOICES, default='o')
    submitted_date = models.CharField(max_length=16)   # timestamp
    photos = models.ManyToManyField(
        Photo, through='PhotoRequest', null=True, blank=True)
